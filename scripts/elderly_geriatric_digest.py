...
{{
  ...
  "groundbreaking": "one or more of: Counterintuitive finding / Overturns prior research / First-in-class human study / Major elder care or policy implication",
  "media_coverage": "{media_note}",
  "summary": "2-4 sentences: what researchers did, who participated (N=X, age range required for geriatric studies), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for older adults, their caregivers, or clinicians. Do NOT imply clinical action.",
  "caveats": "comma-separated flags: small sample (N<100), observational design, single-center, self-reported outcomes, short follow-up, industry funding [name], preprint, secondary analysis, no age breakdown reported, animal or cell study — or 'None identified'",
  "fact_check_note": "Describe any corrections made vs what the abstract actually says, or empty string",
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for geriatric or elder care journalism",
  "pitch_angles": [
    {{
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why it matters for elder care or policy, broader societal significance",
...
- Generate MULTIPLE angles (2-3 max) only when the study genuinely fits different audiences with meaningfully different framings — e.g. a dementia prevention study could pitch differently to STAT News (mechanism/clinical angle) vs McKnight's Senior Living (care protocol angle) vs The Atlantic (personal/policy angle)
- Do not pad with extra angles if one covers it

Rules for content:
- Verify sample size (N), study design, direction of effect against the abstract
- Always note the age range of the study population — a geriatric finding in adults under 65 is a significant caveat
- For neurology: distinguish between risk factors, biomarkers, and clinical outcomes — never imply a gene or exposure "causes" dementia without strong mechanistic evidence
...
relevance_score rubric (1–10): start at 5, then adjust:
  ...
  +2 human subjects with adults 65+ (N≥100)
  +1 clear policy, care, or caregiver hook
  +1 clean design (RCT, longitudinal, large cohort)
  +1 directly addresses dementia, fall prevention, end-of-life quality, pain management, or functional independence
  −1 per major caveat
...
{studies_block}"""
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Major elder care or policy implication", "media_coverage": media_note,
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Elderly & Geriatric Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a3a2e;">Elderly &amp; Geriatric Research Digest</h2>
...
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Elderly &amp; Geriatric Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Elderly & Geriatric Research Digest")
